      ...
      {attempts.map((player, index) => (
        ...
        >
          ...
          <div className="sm:hidden flex flex-col text-center">
            {/* Top Line: Player Name (full-width) */}
            <div className={`font-semibold py-1 px-2 rounded-t-lg text-black overflow-hidden text-ellipsis whitespace-nowrap ${player.name === target.name ? 'bg-green-500' : 'bg-red-500'}`}>
              {player.name}
            </div>
            {/* Bottom Line: Other Details in 6 equal boxes */}
            <div className="flex justify-between gap-0.5 px-2 py-1 bg-gray-200 rounded-b-lg">
              {/* Team Box */}
              <div className={`border rounded bg-white border-gray-300 flex items-center justify-center w-[16.6%] ${getBackgroundColor(player, target, "team")}`}>
                <img src={`/logo/${player.team}.png`} alt={player.countryClub} className="w-4 h-4" />
              </div>
              {/* Country Box */}
              <div className={`border rounded bg-white border-gray-300 flex items-center justify-center w-[16.6%] ${player.country === target.country ? 'bg-green-500 text-black' : 'bg-red-500 text-black'}`}>
                <div className="text-[0.6rem] truncate whitespace-nowrap">{player.country}</div>
              </div>
              {/* Position Box */}
              <div className={`border rounded bg-white border-gray-300 flex items-center justify-center w-[16.6%] ${player.position === target.position ? 'bg-green-500 text-black' : 'bg-red-500 text-black'}`}>
                <div className="text-[0.6rem] truncate whitespace-nowrap">{player.position}</div>
              </div>
              {/* Height Box */}
              <div className={`border rounded bg-white border-gray-300 flex items-center justify-center w-[16.6%] ${getBackgroundColor(player, target, "height")}`}>
                <div className="text-[0.6rem] truncate whitespace-nowrap">{player.height}cm <span className="text-[0.5rem]">{getHint(player, target, "height")}</span></div>
              </div>
              {/* Age Box */}
              <div className={`border rounded bg-white border-gray-300 flex items-center justify-center w-[16.6%] ${getBackgroundColor(player, target, "age")}`}>
                <div className="text-[0.6rem] truncate whitespace-nowrap">{player.age} <span className="text-[0.5rem]">{getHint(player, target, "age")}</span></div>
              </div>
              {/* Number Box */}
              <div className={`border rounded bg-white border-gray-300 flex items-center justify-center w-[16.6%] ${getBackgroundColor(player, target, "number")}`}>
                <div className="text-[0.6rem] truncate whitespace-nowrap">#{player.number} <span className="text-[0.5rem]">{getHint(player, target, "number")}</span></div>
              </div>
            </div>
          </div>
